...
def shortest_path(source, target):
    ...
    while True:
        ...
        """removes node from frontier and adds it to explored set"""
        ...
        for pair in neighbors_for_person(node.state):
            ...
            if pair[1] == target:
                path = []
                path.append((pair[0], pair[1])) #adds current pair to the path list
                while node is not None: #adds all preceding pairs to the path list
                    path.append((node.action, node.state))
                    movie_name = node.action
                    node = node.parent
                return path[::-1]

            """adds Node comprising of pair data to the frontier"""
            if pair[1] not in explored:
                frontier.add(Node(state = pair[1], parent = node, action = pair[0]))
